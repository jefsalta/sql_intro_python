...
def fill():
    ...
    estudiantes = [('Manuel', 4, 3, 'Javier'),
                ('Martina', 11, 6, 'Carlos'), 
                ('Roberto', 10, 3, 'Micaela'),
                ('Gabriel', 6, 1, 'Carlos'), 
                ('Jose', 7, 2, 'Cecilia')
                ]
    conn = sqlite3.connect('secundaria.db')
    c = conn.cursor()
    c.execute("""INSERT INTO estudiante (name, age, grade, tutor) VALUES ('Juan',7,2,'Luis')""")
    c.execute("""INSERT INTO estudiante (name, age, grade, tutor) VALUES ('Pablo',8,3,'Luis')""")
...
def insert(grade):
    ...
    conn = sqlite3.connect('secundaria.db')
    c = conn.cursor()
    c.execute("""INSERT INTO estudiante (name, age) VALUES (?,?)""", grade)
    conn.commit()
    conn.close()
